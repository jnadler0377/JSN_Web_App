...
# Updated: unified CLI (since-days/out) + headless-by-default for servers
...
def dump_artifacts(page, context, *, screenshot: bool = False, save_html: bool = False, trace: bool = False):
    try:
        if screenshot:
            page.screenshot(path=os.path.join(DEBUG_DIR, "pasco_error.png"), full_page=True)
    except Exception:
        pass
    try:
        if save_html:
            html = page.content()
            with open(os.path.join(DEBUG_DIR, "pasco_error.html"), "w", encoding="utf-8") as f:
                f.write(html)
    except Exception:
        pass
    try:
        if trace:
            context.tracing.stop(path=os.path.join(DEBUG_DIR, "pasco_trace.zip"))
    except Exception:
        pass
...
def click_lis_pendens_filed(page_or_frame, humanize_max: int) -> bool:
    ...
    for loc in strategies:
        try:
            if loc.count() > 0:
                ...
                with page_or_frame.expect_navigation(wait_until="networkidle", timeout=15000):
                    loc.click()
                log.info("Clicked 'Lis Pendens Filed'")
                return True
        except Exception:
            pass
    ...
    for target in ["ctl00$Main$btnLisPendens", "Main$btnLisPendens", "btnLisPendens"]:
        try:
            page_or_frame.evaluate(f"window.__doPostBack && __doPostBack('{target}','')")
            page_or_frame.wait_for_load_state("networkidle")
            return True
        except Exception:
            pass

    try:
        page_or_frame.evaluate("document.forms[0] && document.forms[0].submit()")
        page_or_frame.wait_for_load_state("networkidle")
        return True
    except Exception:
        pass
...
def collect_rows_and_links(page, since_days: int):
    items = []
    rows = page.query_selector_all(ROW_SELECTOR) or []
    base = page.url
    cutoff = None
    ...
    for r in rows:
        filed = safe_text(r.query_selector(CELL_DATE))
        case = safe_text(r.query_selector(CELL_CASE))
        a = r.query_selector(CELL_INFO)
        href = a.get_attribute("href") if a else None
        if not (case and href):
            continue
...
def _value_next_to_label(page, label_variants: List[str]) -> str:
    for lbl in label_variants:
        sel = (
            "xpath=//*[self::td or self::th]"
            "[normalize-space(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))="
            f" '{lbl.lower()}']/following-sibling::*[1]"
        )
        loc = page.locator(sel)
        if loc.count() > 0:
            return _norm(loc.first.inner_text())
...
def _collect_all_defendants(page) -> List[str]:
    found: List[str] = []
    rows = page.locator(
        "xpath=//tr[./*[(self::td or self::th) and "
        "contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'defendant')]]"
    )
    for i in range(rows.count()):
        ...
        cell = row.locator(
            ...
            "contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'defendant')]/following-sibling::*[1]"
        )
        if cell.count() > 0:
...
def parse_case_components(case_str: str) -> Optional[Tuple[str, str, str]]:
    ...
    if m:
        return (m.group("year"), m.group("ct").upper(), m.group("seq"))
    return None
...
def civitek_open_portal(page: Page, humanize_max: int):
    ...
    try:
        page.get_by_role("link", name=re.compile(r"public", re.I)).click(timeout=8000)
    except Exception:
        page.click("a:has-text('Public')", timeout=8000)
    page.wait_for_load_state("networkidle")
    human_delay(min(1, humanize_max))
    try:
        page.get_by_role("link", name=re.compile(r"agree", re.I)).click(timeout=8000)
    except Exception:
        page.click("a:has-text('I Agree')", timeout=8000)
    page.wait_for_load_state("networkidle")
    human_delay(min(1, humanize_max))
    try:
        page.get_by_role("tab", name=re.compile(r"case\s*search", re.I)).click(timeout=8000)
    except Exception:
        page.click("text=Case Search", timeout=8000)
    page.wait_for_load_state("networkidle")
    human_delay(min(1, humanize_max))


def civitek_search_and_download(page: Page, year: str, court_type: str, seq: str, out_root: str, humanize_max: int):
    human_delay(min(1, humanize_max))
    page.locator("input[name='caseyear'], input#caseyear").first.fill(year)
    try:
        page.locator("select[name='casetype'], select#casetype").first.select_option(court_type.upper())
    except Exception:
        pass
    page.locator("input[name='caseseq'], input#caseseq").first.fill(seq)

    human_delay(min(1, humanize_max))
    try:
        page.get_by_role("button", name=re.compile(r"search", re.I)).click(timeout=10000)
    except Exception:
        page.click("button:has-text('SEARCH'), input[type='submit'][value*='SEARCH' i]")

    page.wait_for_load_state("networkidle")
    human_delay(min(2, humanize_max))

    targets = [
        re.compile(r"verified\s+complaint\s+to\s+foreclosure\s+mortgage", re.I),
        ...
        for pat in targets:
            if pat.search(text or ""):
                try:
                    ...
                    dest = os.path.join(out_dir, download.suggested_filename or f"{seq}_{i}.pdf")
                    download.save_as(dest)
                    log.info("Downloaded → %s", dest)
                except Exception as e:
                    log.warning("Failed downloading '%s': %s", text, e)
                break
...
def phase1_scrape_and_save(args) -> str:
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            log.info("Harvesting %d info links (since_days=%s)", len(items), args.since_days)
            ...
            for filed, case_no, info_url in items:
                try:
                    ...
                    log.exception("Failed on %s", info_url)
...
def phase2_civitek_download(args, csv_path: str):
    ...
    with sync_playwright() as pw:
        ...
        for case_str in cases:
            ...
            if not parts:
                log.warning("Could not parse Case # '%s', skipping.", case_str)
                continue
            year, ct, seq = parts
            log.info("Civitek search → year=%s, type=%s, seq=%s", year, ct, seq)
            try:
                civitek_search_and_download(page, year, ct, seq, out_root, args.humanize_max)
                human_delay(min(2, args.humanize_max))
                try:
                    page.get_by_role("tab", name=re.compile(r"case\s*search", re.I)).click(timeout=8000)
                except Exception:
                ...
                log.exception("Failed Civitek processing for %s", case_str)
...
def parse_args():
    ...
    p.add_argument("--trace", action="store_true", help="Record Playwright trace to debug/pasco_trace.zip")
...
def main():
    ...
    if args.civitek:
        phase2_civitek_download(args, csv_path)
